...
features_file = os.path.join(script_directory, "adagt/features-grapheme-levenshtein_BS.txt")
assert os.path.exists(features_file), "ADAPT error: Cannot find file " +features_file

num_con_feats = 42    # number of feature values per consonant
num_vow_feats = 27    # number of feature values per vowel
num_punc_feats = 29    # number of feature values per punctuation mark
num_num_feats = 10      #number of feature values per number
...
def align_init():
    ...
    with open(features_file, "r", encoding = "utf-8") as f:
        ...
        for s in file:
            if(s[0] != " " and s[0] != "\n" and s[0] != "\t"):
                ...
                elif len(feats) == num_punc_feats:
                    pu_chars.append(s[0])
                    pu_feats[s[0]] = [float(i) for i in s[1:]]
                elif len(feats) == num_num_feats:
                    nu_chars.append(s[0])
                    nu_feats[s[0]] = [float(i) for i in s[1:]]
    ...
    #----------------------------------------------------------------------
    names = kl_chars + mk_chars + pu_chars +nu_chars
    ...
    #----------------------------------------------------------------------
    # calculate total difference between each punctuation mark pair
    #----------------------------------------------------------------------
    for i in range(len(pu_chars)):
        ci = pu_chars[i]
        fi = pu_feats[ci]
        
        for j in range(i+1, len(pu_chars)):
            cj = pu_chars[j]
            fj = pu_feats[cj]
    ...
    #----------------------------------------------------------------------
    # calculate total difference between each number pair
    #----------------------------------------------------------------------
    for i in range(len(nu_chars)):
        ci = nu_chars[i]
        fi = nu_feats[ci]
        
        for j in range(i+1, len(nu_chars)):
            cj = nu_chars[j]
            fj = nu_feats[cj]
    ...
    #----------------------------------------------------------------------
    # set high differences between vowels/consonants and punctuation marks
    #----------------------------------------------------------------------
    for i in range(len(kl_chars)):
        ci = kl_chars[i];
        for j in range(len(pu_chars)):
            cj = pu_chars[j];
            diff.loc[ci,cj] = 99;
            diff.loc[cj,ci] = 99;
            number_of_diffs.loc[cj,ci] = 99;
    ...
    #----------------------------------------------------------------------
    # set high differences between vowels/consonants/punctuation marks and numbers
    #----------------------------------------------------------------------
    for i in range(len(kl_chars)):
        ci = kl_chars[i];
        for j in range(len(nu_chars)):
            cj = nu_chars[j];
            diff.loc[ci,cj] = 99;
            diff.loc[cj,ci] = 99;
            number_of_diffs.loc[cj,ci] = 99;
    ...
    #----------------------------------------------------------------------
    # set high differences between phonemes/punctuation marks/numbers and word boundary
    #----------------------------------------------------------------------
    ...
    """
    #----------------------------------------------------------------------
    # set high differences between phonemes and punctuation marks
    #----------------------------------------------------------------------
    punctuation_marks = [period_char, exclamation_char, question_char, comma_char, 
                         semicolon_char, colon_char, hyphen_char, bracket_open_char, 
                         bracket_close_char, parenthesis_open_char, parenthesis_close_char,
                         brace_open_char, brace_close_char, apostrophe_char, quotation_mark]
    
    for punc_char in punctuation_marks:
        diff.loc[punc_char,punc_char] = 0
        number_of_diffs.loc[punc_char,punc_char] = 0
        for ci in kl_chars:
            diff.loc[ci,punc_char] = 99
            diff.loc[punc_char,ci] = 99
            number_of_diffs.loc[ci,punc_char] = 99
            number_of_diffs.loc[punc_char][ci] = 99
...
def print_input(ref_line, hyp_line):
    return ref_line, hyp_line
#----------------------------------------------------------------------
    ...
    for i in range(1,lreal+1):
        for j in range(1, lnorm+1):
            ...
            dist[i][j], op = min_func([
                ...
                (dist[i-1][j-1] + float(diff[real_i][norm_j])*0.5),    #substitution
...
#dist_score, nsub, ndel, nins, alnorm, alreal = align_dist("%hallo", "€hoi")
